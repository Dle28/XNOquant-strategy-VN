...
> Chạy lúc: 19/08/2026 – 21/08/2026 (GMT+7) — Mọi số liệu kéo trực tiếp từ API alpha.xnoquant.io
...
| S2-T075 | Threshold 0.75× volume (thay 0.70–2.20×) | **2.86** | 3.11 | 2.40 | -7.1% | 2.06 | 2,304 | 41.5% |
| S2-T125 | Threshold 1.25× volume (thay 0.70–2.20×) | **2.74** | 3.00 | 2.25 | -7.4% | 2.04 | 2,140 | 38.5% |
| | | | | | | | | |
...
### 2.1. Strategy 1 (Equity MID-CAP — short issuers)
...
3. **Gần như toàn bộ alpha đến từ market factors skeleton** (`v*-0.30 + d*-0.30 + y*0.10 + r*0.10`): S1-E (không issuance) đạt 2.84 — tức ~100% Sharpe của bản gốc (2.80) vẫn còn khi bỏ issuance.
...
4. **Threshold filter robust**: đổi dải 0.70–2.20× thành 0.75× và 1.25× đều giữ Sharpe 2.74–2.86 (T075: 2.86, T125: 2.74) — bộ lọc volume không phải nguồn alpha nhạy cảm, đúng vai trò risk filter.
...
### 2.3. Strategy 3 (Equity MID-CAP — long buyback)
...
5. **Vị thế của giao dịch mua lại thực sự có hiệu quả rất nhỏ lẻ**: S3-BP (chỉ 8 trade, Sharpe 0.79 trên mẫu cực nhỏ) — không đủ bằng chứng về buyback anomaly trên VN MID-CAP, chỉ đúng dấu LONG (+).
